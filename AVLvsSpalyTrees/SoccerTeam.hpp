...
public:
  ...
  // copy constructor
  SoccerTeam(const SoccerTeam& st)
    : _name(st._name), _wins(st._wins), _losses(st._losses) {}
  ...
  // overloaded less than and greater than operators
  bool operator<(const SoccerTeam &other) const {return _name < other._name;}
  bool operator>(const SoccerTeam &other) const {return _name > other._name;}
